...
db = ["/home/gotting/projects/fungal_associations_metagenomics/db/sourmash/genbank-2022.03-archaea-k31.zip",
...
for i in db:
    outfile = "{out}/{bn}_{db}.csv".format(out = out_dir, bn = bn, db = os.path.splitext(os.path.basename(i))[0])
    command = "sourmash gather {sig} {db} --threshold-bp 10000 -o {out} 2> {out}_error.txt 1> {out}_screen_output.txt".format(sig = sig_file, db = i, out = outfile)
