...
files_read <- list.files('sourmash/output/output_reads', pattern = 'csv', full.names = TRUE) %>% purrr::map(
  grep, pattern = 'prefetch', invert = TRUE, value = TRUE) %>% unlist()



files_ass <- list.files('sourmash/output/output_assembly', pattern = 'csv', full.names = TRUE) %>% purrr::map(
  grep, pattern = 'prefetch', invert = TRUE, value = TRUE) %>% unlist()
...
data_reads<- lapply(files_read, function(x){
  read_csv(x, col_types = 'nnnnnnnncccnnnncccn') %>%
    mutate(id = paste0(sub("\\..*$", "", basename(x)), "_reads"))
} ) %>% bind_rows() %>%
